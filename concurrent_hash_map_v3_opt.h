#ifndef CONCURRENT_HASH_MAP_V3_OPT_H
...
static ThreadAuditOpt* g_opt_audits = nullptr;
static int g_opt_max_threads = 0;

inline void init_opt_audit(int max_threads) {
    g_opt_max_threads = max_threads;
    g_opt_audits = new ThreadAuditOpt[max_threads]();
}
...
inline void report_opt_audit() {
    ...
    for (int i = 0; i < g_opt_max_threads; ++i) {
        total_global += g_opt_audits[i].global_lock_wait_ns;
        total_bucket += g_opt_audits[i].bucket_lock_wait_ns;
        total_resize_time += g_opt_audits[i].resize_time_ns;
        total_resize_count += g_opt_audits[i].resize_count;
    }

    std::cout << "\n--- PERFORMANCE AUDIT REPORT (V3 OPT) ---" << std::endl;
    std::cout << "1. Global Lock Contention: " << (total_global / 1000000.0) << " ms" << std::endl;
    std::cout << "2. Bucket Lock Contention: " << (total_bucket / 1000000.0) << " ms (OPTIMIZED)" << std::endl;
