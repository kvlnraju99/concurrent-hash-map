#ifndef LOCK_FREE_DYNAMIC_RESIZE_HASH_MAP_H
#define LOCK_FREE_DYNAMIC_RESIZE_HASH_MAP_H
...
// Lock-free chaining hash map with cooperative dynamic resizing.

template <typename K, typename V>
class LockFreeDynamicResizeHashMap {
private:
...
public:
    explicit LockFreeDynamicResizeHashMap(size_t num_buckets = 16,
    ...
    std::optional<V> get(const K& key) const {
        Table* table = const_cast<LockFreeDynamicResizeHashMap*>(this)->ensure_current_target(key);
    ...
    size_t size() const {
        auto* self = const_cast<LockFreeDynamicResizeHashMap*>(this);
...
#endif // LOCK_FREE_DYNAMIC_RESIZE_HASH_MAP_H
